...
interface Files { // for frontend
    name: string;
    url: string;
}
interface FilePath{ //For fetching from sql
    file_path: string;
...
export default async function HomeworkDetailPage({
    ...
    if (homework.file_paths) {
        for (const {file_path, original_name} of homework.file_paths) {
            let url = await redis.get(file_path);
            if (url) {
                ...
                try {
                    ...
                    const presignedUrl = await minioClient.presignedGetObject(
                        ...
                        expirySeconds,
                        {
                            "response-content-disposition": `attachment; filename="${decodeURIComponent(original_name)}"`
                        }
                    );

                    await redis.set(file_path, JSON.stringify({
                        name: original_name,
                        url: presignedUrl
                    }), "EX", expirySeconds);
                    fileArrayPath.push({name: original_name, url: presignedUrl});
                } catch (error) {
                    console.error("Error fetching file from MinIO:", error);
                }


                console.log(file_path, original_name);
            }
